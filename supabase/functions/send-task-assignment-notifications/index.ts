// Invoked every minute by pg_cron (see trigger_send_task_assignment_
// notifications in the migration). Sends exactly one assignment push per
// task — task_notification_deliveries has a unique(task_id) constraint, so
// there is structurally no way for this to become a repeated nag.
import { createClient } from 'jsr:@supabase/supabase-js@2';
import { assertCronRequest, jsonLog, chunk } from '../_shared/cron-auth.ts';

const FN_NAME = 'send-task-assignment-notifications';
...
const PRIVATE_TITLE = 'Tavora task';
const PRIVATE_BODY = 'You have a new task waiting.';
const DETAILED_BODY = 'You have a new task waiting.';
...
Deno.serve(async (req) => {
  ...
  async function handleTick(): Promise<Response> {
    const { data: rows, error: queryError } = await supabase
      ...
      .select('id, task_id, recipient_id, status, attempt_count')
    ...
    // Live re-validation, one row at a time — a task-assignment batch is
    // small (one row per task, ever) so a per-row round trip is simple and
    // fine, unlike the higher-volume recurring reminder pipeline which
    // batches this into one RPC call.
    for (const row of toProcess) {
      const { data: task, error: taskError } = await supabase
        .from('tasks')
        .select('id, title, is_active, connection_id, recipient_id')
        .eq('id', row.task_id)
        .maybeSingle();

      if (taskError) {
        failed++;
        await markFailed(row, 'internal_error', 'task lookup failed, will retry');
        continue;
      }
      if (!task) {
        failed++;
        await markFailed(row, 'task_inactive', 'task_not_found');
        continue;
      }
      if (!task.is_active) {
        failed++;
        await markFailed(row, 'task_inactive', 'task_inactive');
        continue;
      }
      ...
      const body = mode === 'detailed' && task.title ? `"${task.title}" — ${DETAILED_BODY}` : PRIVATE_BODY;
      ...
      const messages = tokens.map((tk: { id: string; expo_push_token: string }) => ({
        ...
        body,
        sound: 'default',
        data: { taskId: task.id, notificationType: 'task_assignment' },
      }));
    ...
    async function markFailed(row: DeliveryRow, code: string, message: string) {
      // No separate "retry-exhausted" terminal status exists for this
      // ledger (unlike reminder_notification_deliveries' 'skipped') — the
      // retry query's own attempt_count < MAX_ATTEMPTS filter is what stops
      // a row from being retried forever; attempt_count itself is what
      // task_notification_health_summary uses to report exhaustion.
      await supabase
        .from('task_notification_deliveries')
        .update({
          status: 'failed',
