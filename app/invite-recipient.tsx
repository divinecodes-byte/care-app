...
import { SelectionCard } from '@/components/AccessiblePrimitives';
...
import { getRelationshipDefinition, RELATIONSHIP_PAIRS, RelationshipPair } from '@/lib/relationshipCore';
...
export default function InviteRecipientScreen() {
    ...
    // Optional -- null ("relationship unspecified") is a fully valid,
    // permanent state, never a placeholder waiting to be filled in. Sent
    // to create_invite_code so the participant can see it (via
    // preview_invite_code) before accepting; see lib/relationshipCore.ts.
    const [relationshipPair, setRelationshipPair] = useState<RelationshipPair | null>(null);
    ...
    async function createInviteCode() {
        ...
        // Required for every NEW invite generated by this current-client UI
        // -- NULL must only ever mean "legacy/unspecified connection" (an
        // old invite predating this requirement, or one created directly
        // via the RPC by a caller that deliberately omits it), never "the
        // organizer skipped the picker." 'other' (Organizer/Participant)
        // remains the explicit generic escape hatch for a relationship
        // that doesn't fit any of the specific pairs -- it is still a
        // conscious selection, not a default.
        if (relationshipPair === null) {
            showAlertOnce(t('inviteParticipant.relationshipRequiredTitle'), t('inviteParticipant.relationshipRequiredMessage'));
            return;
        }
        ...
        const { data: result, error } = await supabase
            .rpc('create_invite_code', { p_existing_connection_id: draftConnectionId, p_relationship_pair: relationshipPair })
            .maybeSingle() as { data: { id: string; invite_code: string; expires_at: string; relationship_pair: string | null } | null; error: { message: string } | null };
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
            ...
            >
                ...
                {/* Relationship (optional) -- captured now so the
                    participant can see it (via preview_invite_code) before
                    accepting, never silently hidden. Disabled once the
                    invite has already been accepted -- create_invite_code
                    only ever writes this to a still-pending row. */}
                <Text style={styles.sectionLabel}>{t('inviteParticipant.relationshipSectionTitle')}</Text>
                <Text style={styles.relationshipHint}>{t('inviteParticipant.relationshipSectionHint')}</Text>
                <View style={styles.chipRow}>
                    {RELATIONSHIP_PAIRS.map((pair) => {
                        const def = getRelationshipDefinition(pair)!;
                        return (
                            <SelectionCard
                                key={pair}
                                ...
                                selected={relationshipPair === pair}
                                disabled={joinedName !== null}
                                compact
                                onPress={() => setRelationshipPair(relationshipPair === pair ? null : pair)}
                            />
                        );
                    })}
                </View>
